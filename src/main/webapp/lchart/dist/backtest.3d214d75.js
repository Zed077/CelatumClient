...
var chart = (0, _lightweightCharts.createChart)(chartContainer);
var runData = null;
var runDataIndex = 0;
...
function populateChart(instIndex) {
    ...
    // Get PnL
    fetch("http://localhost:8080/Celatum/HelloWorld?method=pnl&algorunref=" + runData[runDataIndex].algoRunRef).then(function(response) {
        return response.json();
    }).then(function(data) {
        displayPnL(data);
    }).catch(function(err) {
...
function displayPositions(data) {
    var markers = [
        {
            time: data[0].entryDate,
            position: "belowBar",
            color: "#2196F3",
            shape: "arrowUp",
            text: data[0].entryDesc
        }
    ];
    markers.push({
        time: data[0].closeDate,
        position: "aboveBar",
        color: "#e91e63",
        shape: "arrowDown",
        text: data[0].closeDesc
    });
...
//# sourceMappingURL=backtest.3d214d75.js.map
